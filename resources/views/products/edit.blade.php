...
<div class="py-6 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            ...
            <form action="{{ route($routePrefix . '.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    @if($product->image)
                        <div class="mb-3 flex items-center gap-3">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-20 h-20 object-cover rounded-xl ring-1 ring-gray-200">
                            <div>
                                <p class="text-xs text-gray-400">Gambar saat ini</p>
                                <p class="text-xs text-gray-300">Pilih file baru untuk mengganti.</p>
                            </div>
                        </div>
                    @endif
                    <label for="image" class="group flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-indigo-300 hover:bg-indigo-50/30 transition">
                        ...
                        <span class="mt-1 text-xs text-gray-400 group-hover:text-indigo-500">Klik untuk pilih gambar baru</span>
                ...
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="available" value="1" class="sr-only peer" {{ old('available', $product->available) ? 'checked' : '' }}>
