        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <form method="GET" action="{{ route('kasir.orders.index') }}" class="space-y-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <a href="{{ route('kasir.orders.index', $todayQuery) }}" class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition {{ request('range', 'today') === 'today' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">Hari Ini</a>
                    <a href="{{ route('kasir.orders.index', $yesterdayQuery) }}" class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition {{ request('range') === 'yesterday' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">Kemarin</a>
                    <a href="{{ route('kasir.orders.index', $weekQuery) }}" class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition {{ request('range') === 'this_week' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">Minggu Ini</a>
                </div>
                ...
                <div class="flex flex-wrap items-end gap-2">
                    ...
                    @if(request()->filled('status') || request()->filled('table_number') || request()->filled('date_from') || request()->filled('date_to'))
                        <a href="{{ route('kasir.orders.index') }}" class="inline-flex items-center gap-1 px-3 py-2 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition">
        ...
        <div class="space-y-3">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
                        ...
                        <div class="flex items-start gap-3 flex-1 min-w-0">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span class="text-sm font-semibold text-gray-700">Meja {{ $order->table->table_number ?? '-' }}</span>
                                    @if($isActive)
                        ...
                        <div class="flex items-center gap-2 flex-wrap flex-shrink-0">
                            @if($isUnpaid && !in_array($order->status, ['cancelled']))
                                <a href="{{ route('kasir.orders.show', $order->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm transition">
                            ...
                            @elseif($order->status === 'ready')
                                ...
                                @endif
                            @endif
                            <a href="{{ route('kasir.orders.show', $order->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 shadow-sm transition">
            ...
            @empty
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
                    ...
                    <a href="{{ route('kasir.orders.create') }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm transition">
...
@push('modals')
<a href="{{ route('kasir.orders.create') }}" aria-label="Pesanan Baru"
   ...
   style="position:fixed!important;">
    ...
    <span class="ml-2 text-sm font-semibold whitespace-nowrap opacity-0 max-w-0 group-hover:opacity-100 group-hover:max-w-[120px] transition-all duration-300">Pesanan Baru</span>
</a>
@endpush
</x-app-layout>
