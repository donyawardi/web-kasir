<x-app-layout>
    <x-slot name="header">
        @php
            ...
            $payRoute   = 'orders.process-payment';

            $payment  = $order->payment;
            $isPaid   = $payment && $payment->status === 'success';
            $isUnpaid = !$payment || $payment->status !== 'success';
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route($indexRoute) }}" class="p-1.5 -ml-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition">
    ...
    <div class="py-6 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        @if($isUnpaid && !in_array($order->status, ['cancelled']))
            <div id="payment-section" class="bg-white rounded-2xl border border-yellow-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    <form action="{{ route($payRoute, $order->id) }}" method="POST" id="pay-later-form">
                        @csrf
                        ...
                        <div id="pay-cash-group">
                            ...
                            <div id="pay-quick-cash" class="grid grid-cols-4 gap-1.5 mb-3">
                                @foreach(['1','2','3','4','5','6','7','8','9','0','000'] as $d)
                                    <button type="button" data-digit="{{ $d }}" class="pq-btn py-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 active:bg-gray-200 border border-gray-200 text-gray-800 text-base font-bold transition">{{ $d }}</button>
                                @endforeach
            ...
            (function() {
                ...
                function update() {
                    ...
                    if (isCash) {
                        ...
                        if (received > 0) {
                            changeRow.classList.remove('hidden'); changeRow.classList.add('flex');
                            changeEl.textContent = fmt(change >= 0 ? change : 0);
                            changeEl.className   = 'font-semibold ' + (change >= 0 ? 'text-emerald-600' : 'text-red-500');
                        } else {
                            changeRow.classList.add('hidden'); changeRow.classList.remove('flex');
                        }
                ...
                document.getElementById('pay-later-form').addEventListener('submit', function(e) {
                    ...
                    if (method === 'qris') { form.submit(); return; }
                    ...
                    .then(data => {
                        ...
                        const cancelWrapper = document.getElementById('cancel-wrapper');
                        ...
                        paymentSection.innerHTML =
                            ...
                            '<svg class="w-8 h-8 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></div>' +
                        ...
                        setTimeout(function() { window.location.href = @json(route($indexRoute)); }, 5000);
                    })
                    .catch(() => {
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap items-center gap-3">
            <a href="{{ route($indexRoute) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 text-sm font-semibold rounded-xl shadow-sm transition">
        ...
        <div id="redirect-banner" class="hidden bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
            <p class="text-sm text-blue-800">
                Mengalihkan ke <span class="font-semibold">Daftar Pesanan</span> dalam <span id="countdown-sec" class="font-bold text-blue-900">15</span> detik...
            </p>
            <a href="{{ route($indexRoute) }}" class="text-xs text-blue-600 hover:underline mt-1 inline-block">Klik untuk langsung ke sana</a>
        </div>
...
<script>
(function() {
    const redirectUrl  = @json(route($indexRoute));
    ...
    function startCountdown(seconds) {
        ...
        timer = setInterval(function() {
            remaining--;
            countdownEl.textContent = remaining;
            if (remaining <= 0) { clearInterval(timer); window.location.href = redirectUrl; }
        }, 1000);
    }
    ...
    if (cetakBtn) {
        cetakBtn.addEventListener('click', function() {
            window.open(@json(route('orders.receipt', $order->id)) + '?autoprint=1', 'receipt', 'width=420,height=600');
        });
    }

    window.addEventListener('message', function(e) {
        if (e.data === 'receipt-printed') startCountdown(15);
    });
})();
</script>
</x-app-layout>
